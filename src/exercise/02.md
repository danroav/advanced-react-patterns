...
Compound Components centralize state of child components and return a copy of the children components with the state needed for them. The design pattern stipulates that Child components should call Compound Component functions based on the state received or extended.

Compound Component return cloned child components with React.cloneElement(child,{newProps}).

Child components that are DOM elements (div, span, label) will receive the new props and generate a console error if there are custom name props. Only provide props to the children needed and not everyone

Component type can be accessed from component.type, either a function for function componentes or a string with the DOM element name
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=02%3A%20Compound%20Components&em=a.b%40c).
